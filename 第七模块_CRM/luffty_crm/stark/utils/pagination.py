...
class Pagination(object):
    def __init__(self, current_page, all_count, base_url, query_params, per_page=20, pager_page_count=11):
        ...
        pager_count, b = divmod(all_count, per_page) # 取整和取余
        if b != 0: # 如果余数不为0，取得的整数就加一
            pager_count += 1
        self.pager_count = pager_count

        half_pager_page_count = int(pager_page_count / 2) # 页面上显示页码数量的一半
    ...
    def page_html(self):
        ...
        if self.pager_count < self.pager_page_count: # 总页码小于要显示的页码按钮数量
            pager_start = 1
            ...
            if self.current_page <= self.half_pager_page_count:
                ...
                if (self.current_page + self.half_pager_page_count) > self.pager_count:
                    pager_end = self.pager_count # 页面显示页码的最后一个页码
                    pager_start = self.pager_count - self.pager_page_count + 1 # 页面显示页码的第一个页码
                else:
