...
class ConsultantRecordAddModel(StarkModelForm):
...
class ConsultantRecordHandler(PermissionHandler, Handler):
    ...
    has_add_btn = True
    list_display = ['note', 'consultant', 'date', Handler.display_edit, Handler.display_del]
    list_template = ['consultant_record.html']
    model_form_class = ConsultantRecordAddModel
    ...
    def get_queryset(self, request, *args, **kwargs):
        ...
        return self.model_class.objects.filter(customer_id=customer_id, consultant_id=current_user_id)
    ...
    def change_object(self, request, pk, *args, **kwargs):
        ...
        obj = models.ConsultRecord.objects.filter(pk=pk, customer_id=customer_id,
    ...
    def del_object(self, request, pk, *args, **kwargs):
        ...
        current_user_id = request.session['user_info']['id']
        customer_id = kwargs.get('customer_id')
        del_obj = models.ConsultRecord.objects.filter(pk=pk, customer_id=customer_id,
