...
class CourseRecordModel(StarkModelForm):
...
class StudentRecordModel(StarkModelForm):
...
class ClassRecordHandler(PermissionHandler, Handler):
    ...
    has_add_btn = True
    model_form_class = CourseRecordModel
    ...
    def get_urls(self):
        ...
        patterns = [
            ...
            re_path(r'^edit/(?P<class_id>\d+)/(?P<pk>\d+)/$', self.wrapper(self.edit_view), name=self.get_edit_url_name),
    ...
    def action_multi_init(self, request, *args, **kwargs):
        ...
        for course_record in course_record_id_list:
            ...
            course_record_obj = models.CourseRecord.objects.filter(id=course_record, class_obj_id=class_id).first()
            ...
            if not student_record_obj:
                continue
            
            student_record_obj_list = [models.StudyRecord(student_id=stu.id, cuorse_record_id=course_record) for stu in 
                                       student_obj_list]
    ...
    def display_attendance(self, obj=None, is_header=None, *args, **kwargs):
        ...
        if is_header:
            return '考勤'
        name = '%s:%s' % (self.stark_site.namespace, self.get_del_url_name('attendance'))
    ...
    def attendance_view(self, request, course_record_id, *args, **kwargs):
        ...
        student_model_formset = modelformset_factory(models.StudyRecord, form=StudentRecordModel, extra=0)
