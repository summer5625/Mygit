...
class Request:
    ...
    def _parse(self):
        ...
        # 在这里：配置的类rest_framework.negotiation.DefaultContentNegotiation调用了
        # select_parser(request, [配置的解析器类的实例化对象])方法
        # parser就是通过前端传过来的ContentType数据类型和我么所配置的所有解析器去匹配，匹配上了相应的解析器，就将改解析器返回
        # 所以parser就是匹配好的解析器
    ...
    def _authenticate(self):
        ...
        in turn.
        """
        # 循环该列表[auth() for auth in self.authentication_classes]
        # 列表中是实例化的所有认证类
        for authenticator in self.authenticators:
            try:
                # 配置的认证类中必须要实现uthenticate()方法
                # self是request
