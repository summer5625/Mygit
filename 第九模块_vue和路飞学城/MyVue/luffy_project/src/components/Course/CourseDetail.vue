        ...
        <div class="course-price">
            <div class="container">
                ...
                <ul class="course-price-item" >
                    <li v-for="(item, index) in price" :key="item.id" :class="{active:index===currentIndex}"
...
export default {
  ...
  data(){
    return{
      coursedetailtop:{}, //课程详情顶部
      coursedetail:{}, //课程详情数据
      currentIndex:null,
      price:[
        {id: 1, valid_period: 30, valid_period_name: '1个月', price: 99},
        {id: 2, valid_period: 60, valid_period_name: '2个月', price: 199},
        {id: 3, valid_period: 90, valid_period_name: '3个月', price: 299},
        {id: 4, valid_period: 180, valid_period_name: '6个月', price: 399},
      ], //视频里api不能用了，只能自己造一下课程价格详情数据
    }
  },
  ...
  methods:{
      ...
      if (this.price[this.currentIndex]){
        if (window.localStorage.getItem('access_token')){
          ...
          let course = {
  					...
  					validPeriodId:this.price[this.currentIndex].valid_period
  				};
          console.log(course);
          this.$http.shopCart(course).then(res=>{
            console.log(res);
            if (res.error_no===0){
    ...
    getCoursedetailtop(){
      this.$http.courseDetailTop(this.$route.params.detailId).then(res=>{
    ...
    //课程概述数据，原来api不能用，新api获取数据和原来不一样，暂时放置
    courseDetail(){
